...
## Track 10: Decay-product composition and cross-shear boost

Track 10 explored two questions:

1. Can the muon be built by composing its decay products' quantum
   numbers?  (μ⁻ → e⁻ + ν̄_e + ν_μ ⇒ n_μ = n_e + n_ν₁ + n_ν₂)
2. Can cross-shear σ_eν, generated by the ν companion's presence,
   boost the effective e-sheet mass enough to reach the muon?

### F66. Decay-product composition gives ~0.511 MeV, not 105.66 MeV

When the muon's decay products are composed as summed quantum
numbers — n_μ = (1, 2, n₃, n₄, 0, 0) for all integer (n₃, n₄)
— the resulting energy from the metric is always 0.511 MeV,
regardless of the ν dressing.  Even (1, 2, 1000, 1000, 0, 0)
gives 0.510999 MeV.

**Why:** the ν sheet is 7.1 × 10⁶ times larger than the e sheet.
Each ν winding adds ~3 × 10⁻⁸ MeV to the mode energy.  The
p sheet is ~1,300× smaller than the e sheet, so p windings add
~279 MeV per unit — too coarse.  No integer combination of ν and
p windings can bridge 0.511 → 105.66 MeV.

The number of decay-product components is irrelevant: whether the
muon is "electron + 2 neutrinos" or "electron + 5 neutrinos", only
the total ν winding enters the energy formula.

### F67. Cross-shear σ_eν DOES boost e-sheet mass via Schur complement

At fixed L_ring_e (e-sheet size unchanged), increasing σ_eν modifies
the dimensionless metric G̃ and its inverse.  The e-sheet block of
G̃⁻¹ changes through the Schur complement (the matrix-inverse
back-reaction of the e-ν coupling on the effective e-sheet mass).
The (1,2) mode energy climbs:

| σ_eν | E(1,2,0,0,0,0) | Boost |
|------|-----------------|-------|
| 0.00 | 0.511 MeV | 1.0× |
| 0.20 | 0.549 MeV | 1.07× |
| 0.35 | 0.664 MeV | 1.30× |
| 0.45 | 0.900 MeV | 1.76× |
| 0.50 | 1.275 MeV | 2.49× |
| ≥0.55 | singular | metric fails |

**The mechanism is real** — cross-shear changes the effective
e-sheet mass without resizing the sheet.  But the maximum boost
before the metric loses positive-definiteness is **2.5×**.  The
muon requires **207×** (= m_μ/m_e).

### F68. ν windings contribute zero — they are a catalyst, not a contributor

At every σ_eν value, E(1,2,0,0,0,0) = E(1,2,1,1,0,0) =
E(1,2,2,3,0,0) to all displayed digits.  The ν windings do not
change the mode energy because n_ν/L_ν ≈ 0.

**Physical interpretation:** the ν companion's role is NOT to add
energy directly.  It is to **generate** the cross-shear σ_eν (or
some more general geometric coupling) that then modifies the
effective e-sheet mass.  The ν windings are a trigger, not a
payload.  This is consistent with the free electron being
unaffected: it has n₃ = n₄ = 0, so σ_eν has nothing to couple
to, and the bare electron stays at 0.511 MeV.

### F69. Paths forward: what would reach the muon and pions?

Tracks 8–10 have exhaustively tested every avenue available in
the current MaD formalism.  The muon and all three pions remain
unreachable.  Here is what we know and what remains open.

**What we know:**

1. The 5–248 MeV desert is structural: zero Q = −1, spin-½ modes
   across all 4 sign branches, 21 σ_eν values, and 3 M candidate
   6-tuples.  No single mode exists there.  (F56)
2. Pair sums also fail for the desert: pairs land at either ≤ 10 MeV
   (two anomalously light) or ≥ 248 MeV (one at spectral floor).
   No pair reaches 105–140 MeV.  (F62–F63)
3. Composing decay products as summed quantum numbers gives the same
   6-tuples already searched — the ν sheet is too large to contribute
   energy.  (F66)
4. Cross-shear σ_eν CAN boost the e-sheet mass without resizing
   the sheet, but the current metric structure allows only 2.5×
   before singularity.  Need 207× for the muon.  (F67)
5. ν windings don't add energy directly — they act as a geometric
   trigger/catalyst.  (F68)
6. The charged kaon was rescued at 0.73% as a spin-singlet pair
   of two ~249 MeV modes.  (F61)
7. Charged pions are topologically forbidden as single modes AND
   fall in the desert as pairs.  They face two independent barriers.
   (F58, F62)

**Possible paths to the muon (105.66 MeV):**

**(A) Mode-dependent cross-shear (most conservative).**
The free electron sees σ_eν = 0 (no ν companion).  The muon's ν
companion generates an effective σ_eν that modifies the e-sheet
metric.  This is the mechanism tested in F67, which works in
principle but is quantitatively insufficient (2.5× vs 207× needed).
To reach 207×, the metric parameterization must be extended:

- **Richer coupling structure:** replace the single scalar σ_eν
  with a 2×2 matrix (independent tube-tube, tube-ring, ring-tube,
  ring-ring couplings).  A structured coupling might avoid the
  singularity that currently caps the boost at 2.5×.
- **Multiple parameters shifting:** the ν companion might modify
  not just σ_eν but also the effective ε_e and s_e within the
  compound geometry.  Three co-moving parameters could produce
  207× where one alone manages only 2.5×.
- **Nonlinear metric:** the current G̃ is linear in σ_eν
  (additive correction).  A multiplicative or exponential coupling
  — where σ_eν enters through the lattice matrix B rather than
  as an additive correction to G̃ — would have a different
  singularity structure.

**(B) Decay-product constraint on Engine B.**
The muon's decay μ⁻ → e⁻ + ν̄_e + ν_μ identifies which ν modes
are the companions.  This constrains the back-reaction to a
one-parameter problem: find the coupling λ such that the specific
ν companion produces E = 105.66 MeV for the (1,2) electron base.
If the same λ then predicts the tau mass for a different ν
companion (ν_τ), that is a genuine prediction, not a fit.

**(C) Variational / action-based approach (most ambitious).**
Write an action functional S[ε, s, σ; n] for the fabric that
includes elastic tension for departures of geometry from the
"free" values, and a coupling between mode windings and geometry.
Stationary points δS = 0 give all self-consistent modes — linear
and compound — in one unified framework.

**Possible paths to the pion (135–140 MeV):**

The pion faces an additional barrier beyond the desert: the
topological spin-charge constraint forbids Q = ±1, spin = 0 as
a single 6-tuple.  Any solution must address both barriers.

**(D) Spin-singlet pair with back-reaction.**
If back-reaction (path A/B/C above) can create ~70 MeV components
(half the pion mass), then a spin-singlet pair of two spin-½
modes could reach the pion.  This requires the back-reaction to
populate the desert with intermediate-mass modes first.

**(E) Pion = muon + neutrino.**
The decay π⁺ → μ⁺ + ν_μ suggests the pion is the muon plus one
more ν winding.  If the muon is solved via path A/B/C, the pion
follows: add the extra ν_μ winding and compute the new compound
mass.  This gives a built-in prediction chain: solve the muon,
predict the pion.

**(F) Chiral pairing or extended topology.**
If none of the above work, the pion may require physics not in
the current three-sheet framework — e.g. chiral modes (left/right
pairs on the same sheet whose spins cancel), or a fourth compact
dimension that relaxes the spin-charge constraint.

**Priority ordering:** A → B → E → D → C → F.  Path A is the
smallest extension of the current formalism.  Path B uses the
decay-product constraint to make A a one-parameter problem.
Path E piggybacks on the muon solution.  Path C is the most
principled but requires new mathematical framework.  Path F is
a last resort.

### Track 10 status

**Complete.**  Scripts: `scripts/track10_decay_composition.py`,
`scripts/track10b_sigma_boost.py`.
Result: decay-product composition gives 0.511 MeV (ν sheet too
large); cross-shear boosts e-sheet mass in principle (correct
mechanism) but current metric caps at 2.5× vs 207× needed.
Paths A–F documented for future work.

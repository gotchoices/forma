# R31 Findings: The origin of α and the nature of atomic binding

Study: `R31-alpha-derivation/`
...
The within-plane shears s₁₂ and s₅₆ (which determine α)
have no measurable effect on the total Casimir energy:
log₁₀(Z) = 113.536 at ALL shear values tested.
...
There is NO interior minimum.  The vacuum energy of the
electron sheet does not select a preferred α.
...
The missing ingredient is DYNAMICS — the energy cost of
maintaining the shear.  The shear distorts the torus;
this distortion costs energy.  Balancing distortion energy
against Casimir energy could create a minimum (moduli
stabilization), but this requires a gravitational or
elastic energy functional not yet in the model.

### F7. Four candidate mechanisms for selecting α

1. **Moduli stabilization** — distortion energy balances
   Casimir energy (requires gravitational action)
2. **Back-reaction of charge** — the charged mode distorts
   the metric, self-consistently determining s (requires
   coupled Einstein-Maxwell on the torus)
3. **Observable constraint** — precision measurement pins
   r_e, α follows (near-term but doesn't explain WHY)
4. **Topological constraint** — none found; all s values
...
### F9. The merger barrier is 60,000× the binding energy

The Coulomb binding energy of hydrogen (13.6 eV) vs the
merger cost to form a neutron (782,000 eV): a ratio of
~57,500.  Atoms are extraordinarily stable against
collapse into neutrons.

At room temperature (kT = 0.026 eV), the merger threshold
is ten million times too high.  Only extreme environments
(supernovae, neutron stars) can drive inverse beta decay.

### F10. Two-tier physics: complete picture
...
| Separation | Physics |
|-----------|---------|
| r > a₀ (53,000 fm) | Free particles |
| r ~ a₀ | Hydrogen atom (Coulomb) |
| r ~ L₁ (32,000 fm) | Compact geometry visible |
...
| Mode | Energy | Q | Description |
|------|--------|---|-------------|
| (0,0,0,0,1,2) | 938.3 MeV | +1 | proton |
| (0,−2,1,0,0,2) | 939.6 MeV | 0 | neutron |
| (2,−2,0,0,2,2) | 939.9 MeV | 0 | 2e+2p merger |
| (2,−4,0,0,2,4) | 1876.5 MeV | 0 | doubled merger |
...
## Track 4 — Hydrogen Lamb shift constrains r_e

### F15. The naive KK Yukawa correction is catastrophically large

Computing the Yukawa correction to the hydrogen 2S-2P
splitting from KK massive modes:

| r_e | M₁a₀ | ΔE_Lamb (MHz) | vs. Lamb shift |
|-----|-------|---------------|----------------|
| 0.3 | 120.5 | −342,000 | 324× too large |
| 1.0 | 62.9 | −1,227,000 | 1,160× too large |
| 6.6 | 10.4 | −35,661,000 | 33,700× too large |
...
### F16. The correction is power-law, not exponential

Despite M₁a₀ reaching 120 at small r_e, the correction
does NOT fall exponentially.  It falls as a power law
(~1/(M₁a₀)²) because the 2S wavefunction has |ψ(0)|² ≠ 0
and samples the Yukawa potential at the origin, where it
behaves identically to Coulomb.

### F17. The naive KK Yukawa coupling is ruled out

The Lamb shift is measured to ±0.009 MHz precision.  The
naive KK Yukawa exceeds this by at least 10⁵.  This rules
...
gives ~8% correction).  The correction IS large if naive
coupling is assumed, but the coupling must not be naive.


## Track 6 — Multi-electron atoms and the Pauli principle
...
### F23. Why α cannot be derived from this model (yet)

The model computes kinematics (spectra, energies) but not
dynamics (what selects the geometry).  To predict α requires:

1. An energy functional V(r_e, s₁₂) that INCLUDES the cost
   of maintaining the shear (gravitational/elastic energy),
   not just the vacuum energy.

2. A minimum of V at specific (r_e, s₁₂) → predicted α.
...
| Does Lamb shift pin r_e? | Cannot — naive Yukawa ruled out | 4 |
| Is r_e per-electron? | NO — geometric constant | 6 |

### Free variables after R31

| Variable | Status | Constraint |
|----------|--------|------------|
| r_e | FREE | 0.26 < r_e < 68.5 (R30) |
| r_ν | Weakly constrained | r_ν ≥ 3.2 (R26) |
| r_p | FIXED | 8.906 (neutron mass, R19) |
| s₁₂ (→ α_e) | Input | α = 1/137.036 (measured) |
| s₅₆ (→ α_p) | Input | Same α assumed |
| σ_ep | FIXED | −0.09064 (R19/R27) |
...
To derive α, the model needs DYNAMICS — an energy functional
that penalizes certain geometries and selects the physical one.
This is a fundamentally different kind of calculation from
anything attempted so far.


## Q&A: Why is r_p fixed but r_e free?
...
- Muon mode (−1,+5,0,0,−2,0) = 105.658 MeV → pins L₅

Two particles, two circumferences, both determined.
r_p = L₅/L₆ = 8.906 follows.  The geometry CONSTRAINS the
proton-muon mass relationship; the specific value of r_p
comes from the measured masses.

### Q: Is there an analogous mechanism for r_e?

No — and the reason is structural.  The electron tube
(L₁ ~ 32,000 fm) contributes negligibly to every known
particle's mass.  Energy per quantum number step:

| Dimension | L (fm) | Energy per step |
|-----------|--------|-----------------|
| n₅ (proton tube) | 23.7 | 52,393 keV |
| n₆ (proton ring) | 2.66 | 466,611 keV |
| n₂ (electron ring) | 4,880 | 254 keV |
| n₁ (electron tube) | 32,209 | 38 keV |

The proton tube step is **1,400× larger** than the electron
tube step.  Every particle with n₁ ≠ 0 gets its mass
overwhelmingly from the proton sheet:

| Particle | n₁ | Electron tube fraction |
|----------|----|-----------------------|
| electron | 1 | 0.12% |
| muon | −1 | 0.003% |
| tau | −1 | 0.0002% |
...
### Q: Does the muon help pin r_e?

The muon has n₁ = −1, so it USES the electron tube.
But its energy is 99.997% determined by the proton tube
(n₅ = −2).  When r_e is swept from 1.0 to 50.0, the muon
...
noise at the MeV scale.  The ONLY particle whose energy
significantly involves the electron sheet is the electron
itself — one equation, two unknowns (L₁, L₂).

To pin r_e, you would need one of:

1. **An undiscovered ~keV particle** with large n₁ and small
   proton windings — no known particle lives there.
2. **A precision measurement** sensitive to L₁ (Lamb shift,
   g−2) — but Track 4 showed the naive Yukawa coupling is
   ruled out by a factor of 10⁵.
3. **A geometric principle** (Casimir, moduli potential) —
   but Track 3 showed Casimir alone has no minimum.

### Q: Why did it work for r_p but not r_e?

Because the proton sheet is **loud** — its energy scales
(52–467 MeV per step) dominate the mass spectrum.  Many known
particles have significant proton-sheet windings.  The electron

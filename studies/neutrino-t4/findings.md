...
### F47. The charge radius selects the aspect ratio

The experimental proton charge radius r_ch = 0.8414 fm is
reproduced at TWO aspect ratios:

| Solution | ε     | s_KK     | R (fm)  | a (fm)  |
|----------|-------|----------|---------|---------|
| A        | 0.328 | 0.1758   | 0.747   | 0.245   |
| B        | 0.965 | 0.0672   | 0.461   | 0.445   |

Solution A: thin tube (a < R), the charge radius is dominated
by R ≈ 0.75 fm ≈ 89% of r_ch.

Solution B: nearly square torus (a ≈ R), the tube spread adds
comparably to R.  This is close to ε = 1.

Both are far from S2's ε = 6.6.  At ε = 6.6, r_ch = 4.4 fm
(5× too large).


### F48. The ring model cannot match experiment

The pure ring model (r_ch = R, all charge at radius R) gives
R_max ≈ 2λ̄_p = 0.421 fm as ε → ∞.  This is only 50% of the
experimental value.  The tube structure of the torus is essential
for the charge radius.


### F49. The charge radius constrains ε to ≲ 1

For ε > 1, r_ch grows rapidly (∝ ε).  At ε = 2, r_ch ≈ 1.4 fm.
The experimental value 0.841 fm requires ε < 1 (approximately),
meaning the tube is narrower than the ring (a < R).

This is a NON-TRIVIAL constraint.  Previously, the aspect ratio
was completely free (R19 F15 — one-parameter family).  The charge
radius measurement pins it to ε ≈ 0.33 or ε ≈ 0.97.


### F50. Predicted electron charge radius

If the electron has the same aspect ratio as the proton, its
charge radius scales as:
    r_ch(e) = r_ch(p) × m_p/m_e ≈ 1545 fm ≈ 1.5 pm

This is below the current experimental upper bound (~2.2 fm from
high-energy e⁻e⁻ scattering) and far too small to measure with
current technology.  The electron "looks" point-like in this model
because its Compton wavelength (386 fm) is much larger than its
geometric size.


### F51. Sensitivity to ε

The charge radius is moderately sensitive to ε near the match:
a ±5% change in ε produces a ±6–10% change in r_ch.  This means
the experimental uncertainty in r_ch (currently ±0.019 fm, ~2%)
constrains ε to about ±1%.


### F52. Caveat on the charge distribution model

The cos²θ weighting assumes:
(a) the mode's field pattern is well-approximated by exp(i(θ+q_eff φ))
(b) the two cos θ factors are independent
(c) the torus is embedded with standard geometry

A full electromagnetic calculation on the embedded torus —
solving for the actual 3D fields generated by the mode — could
modify the θ-dependence and shift r_ch by O(1) factors.  This
is a first-order estimate, not a precision prediction.

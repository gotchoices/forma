...
- Is a slot a viable alternative to internal shear as a sole mechanism for producing charge?
...
- If so, are two slots viable?  It may be difficult to get two slots that don't kill the fundamental.
- What about a single slot on the inside equator on a horn torus?  Should eliminate electric dipole?
- Is our current model (straight wave equation shear, but no slot) uniform?  Or is it a dipole?  One earlier study seemed to indicate that it might not be uniform.
...
- Is there any chance that 1/137 produces expected charge and moment and 1/0.036 produces the additional anomalous part?
...
- Calculate the wave function on the slotted torus sheet given known inputs.
...
- Determine the resulting frequency response of the cavity.  What modes will still ring?
- If too many open variables to determine slot dimensions, try setting it to desired height to block unwanted ghost modes (there is one in particular that is lighter than the electron and problematic).

...
| [`grid-lab.html`](grid-lab.html) | GRID | Node/edge lattice workbench implementing grid-duality's Scattering model: each edge has two registers (one per end); inhale applies S = (2/N)·J − I at each node, exhale swaps registers across each edge. 1D linear array and 2D hex/wye sheet, with independent per-axis periodicity; 1D periodic + visual wrap rolls into a ferris-wheel ring. Edges render as heat-mapped halves; periodic flat layout shows half-edge stubs. Click an edge half to edit its register. Profile save/load. 3D diamond / 2D torus forthcoming. |

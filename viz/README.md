...
| [`proton-lab.html`](proton-lab.html) | SHEET-PROTON | Clover-shaped corrugated torus from `projects/sheet-proton/work/clover-quarks.md`: cross-section is a 3-lobe + 3-saddle clover under D₃ symmetry (kissing circles), swept with a chiral twist τ (in 1/3 steps) and continuous shear s. Adjustable ring/lobe/saddle radii. Quark/baryon presets (u, d, proton uud, neutron udd) highlight constituent lobe/saddle arcs and overlay closed winding paths. 2D cross-section preview + named profile save/load. |
| [`tube-lab.html`](tube-lab.html) | TUBE | Generalized smooth-cross-section corrugated tube. Polar curve r(φ) = R_c·[1 + a₁·cos(Nφ) + a₂·cos(2Nφ)] with adjustable lobe count N (2–8), fundamental amplitude a₁, second-harmonic a₂ (controls valley depth / concavity), mean radius R_c, ring radius R_major, continuous twist τ (with snap-to-closure k/N), and shear. C^∞ curvature shading (warm = convex lobes, cool = concave saddles). Presets for circle, electron-tube bilobe (N=2, τ=2), rounded N-gons, smooth/deep clovers (N=3). (n_θ, n_φ) path overlay with multi-copy and profile save/load. |
| [`grid-lab.html`](grid-lab.html) | GRID | Node/edge lattice workbench implementing grid-duality's Scattering model: each edge has two registers (one per end); inhale applies S = (2/N)·J − I at each node, exhale swaps registers across each edge. 1D linear, 2D hex/wye, 3D diamond, with independent per-axis periodicity. 1D ring (ferris wheel) and 2D torus (with Ny = 2k·Nx closure) when Wrap is on. Presets include WvM (1,2) torus-knot standing wave for 2D. Edges render as heat-mapped halves; periodic flat layouts show half-edge stubs. Click an edge half to edit its register. Profile save/load. |

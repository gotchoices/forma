...
### 1a. Intermediate analytical level (not realized in nature)

The level table jumps directly from level 0 (non-compact, scalar
phase) to level 1 (S¹ aleph → photon).  Between them sits a
theoretical rung that nature does not appear to use, but which
clarifies *what each step contributes*:

| Level | Substrate | Vertex degree | Output |
|:-----:|:--|:-:|:--|
| 0 | bare aleph (no extension) | — | static scalar phase |
| **0.5** | **1D chain of alephs (linear array)** | **N = 2** | **propagating scalar quantum ("sound")** |
| 1 | 2D grid of alephs (branching network) | N = 3 | photon (vector) |

The 0 → 0.5 step adds **propagation** (a wave can move through
space; before, the phase was static at a point).  The 0.5 → 1
step adds **polarization** (branching vertices give the wave a
vector index — the "which outgoing edge?" choice at each node
*is* the gauge field's vector index in the continuum limit).

These two features are structurally separable.  Compactification
on S¹ does not, by itself, give the photon; it gives a scalar
quantum that acquires polarization only when the network has
branching connectivity (N ≥ 3).  In other words:

> **Light = a propagating scalar quantum, plus a polarization
> label generated by lattice branching.**

Strip the branching, and what remains is "sound" in the abstract
sense — a longitudinal scalar wave with phase, frequency,
energy, and momentum, but no transverse polarization.  Particle
sheets in MaSt skip this rung and connect aleph directly to the
2D grid (the wye junction is the lowest realized branching).
The intermediate rung is included here for analytical hygiene:
it isolates *what the wye does* from *what the S¹ does*, and
makes clear that the photon's vector character is a consequence
of vertex degree, not of compactification per se.

This refinement does not alter the level enumeration of section
1 — levels 1, 2, 3 remain the realized rungs.  But it explains
*why* level 1's output is rank-1 (vector) when its input is
rank-0 (scalar): the branching at level 1 is doing the rank
promotion, separately from what the compactification is doing.
A standard-physics analog: in 1+1D scalar field theory, kinks
behave as fermions under bosonization (Coleman–Mandelstam), so
the analytical level may also host topological precursors of
the level-2 (mass) content — speculative, but worth flagging.

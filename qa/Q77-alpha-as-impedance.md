...
**Related:** [Q18](Q18-deriving-alpha.md) (deriving α),
[Q29](Q29-variational-principle-alpha.md) (variational α),
Q75 (contingency of α), [Q76](Q76-origin-of-metric-signature.md)
(origin of dimensional character), R31 (alpha derivation study),
[Q32](Q32-energy-geometry-fundamentals.md) (energy/geometry fundamentals)
...
When a wave crosses from one medium to another with
different wave speeds, the transmitted fraction depends on
the impedance ratio.  In classical EM:

- The vacuum impedance is Z₀ = √(μ₀/ε₀) ≈ 377 Ω
- At an impedance boundary, the transmitted power fraction
  is T = 4Z₁Z₂/(Z₁ + Z₂)²
...
In the current model, the shear s tilts one compact
...
A sheared torus is metrically anisotropic: propagation
speed depends on direction.  The tube and ring are no
longer orthogonal, so a photon moving along the tube has
a component of motion along the ring.  This directional
anisotropy IS an effective speed-of-light difference —
not a uniform slowdown, but a tilt that makes the two
internal directions experience different effective speeds.

The magnitude of this anisotropy determines α.  So the
shear mechanism already implements the impedance-mismatch
idea — it's just not usually described that way.
...
|----------|--------|-------|
| Casimir energy minimum | No minimum found | R31 Track 3 |
| Moduli stabilization | Requires unknown dynamics | R31 F7, F23 |
...
2. **Energy conservation across domains**: The total
   energy (compact standing waves + spatial radiation)
   must be conserved.  The "exchange rate" between the two
   forms of energy might be fixed by dimensional analysis
   or by the requirement that the partition is stable.
...
R31 Track 5 found that the Bohr radius (~53,000 fm) and
the electron tube circumference (~32,000 fm) are within
a factor of 1.7 of each other.  Both length scales are
set by m_e but scaled differently:

- Bohr radius: a₀ = 1/(α m_e) (in natural units)
- Electron tube: L₁ = 2πr_e/m_e (depends on r_e)
...
1. Ten raw dimensions acquire character (Q76)
...
## Relationship to natural constants

The three fundamental conversion factors map onto the three
types of dimensional interface:

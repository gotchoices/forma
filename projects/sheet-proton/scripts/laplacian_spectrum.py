...
def hill_eigenvalues(k_v: float, eps: float, chi: float, N: int, n_eigs: int) -> np.ndarray:
    ...
    Matrix elements (working in R_major = 1 units, c = ε):
        ⟨ψ_q|K|ψ_p⟩ = (p²/c²) δ_{q,p}
                    + (p·q/c²) · a_{q-p}      [coupling via P_x]
                    + k_v² · (1/w)_{q-p}      [coupling via 1/(1+P_x)]
        ⟨ψ_q|M|ψ_p⟩ = δ_{q-p,0} + a_{q-p}     [w = 1 + P_x]

    where a_k = (1/2π) ∫ P_x e^{-iku} du and (1/w)_k = (1/2π) ∫ (1/w) e^{-iku} du
    are computed by FFT of the profile sampled on N points.
    ...
    # Build profile and compute Fourier transforms
    c = eps
    r_lobe = eps / (2.0 + chi)
    r_saddle = chi * r_lobe
    ...
    # Numerical Fourier coefficients (1/N · Σ f_j e^{-ikj·2π/N} gives Fourier coefficient at k)
    w_coefs = np.fft.fft(w) / N        # w_coefs[k] = w_k for k = 0, 1, ..., N-1 (cyclic)
    invw_coefs = np.fft.fft(inv_w) / N

    def w_coef(kappa: int) -> complex:
        return w_coefs[kappa % N]

    def invw_coef(kappa: int) -> complex:
        return invw_coefs[kappa % N]

    def a_coef(kappa: int) -> complex:
        # P_x = w - 1, so a_κ = w_κ for κ ≠ 0, and a_0 = w_0 - 1
        v = w_coef(kappa)
        if kappa == 0:
            v = v - 1.0
        return v

    # Build dense Hermitian matrices K and M
    ...
    for i, p_i in enumerate(p_values):
        for j, p_j in enumerate(p_values):
            ...
            K[i, j] = (p_j**2 / c**2) * (1.0 if kappa == 0 else 0.0) \
                    + (p_j * p_i / c**2) * a_coef(kappa) \
                    + (k_v**2) * invw_coef(kappa)
            M[i, j] = w_coef(kappa)
...
def predicted_zeroth_order(k_v: float, eps: float, n_eigs: int) -> np.ndarray:
    """Analytical flat-limit prediction: μ² = k_v² + (p/ε)² for integer p ∈ ℤ.

    This is the prediction we want to validate against, NOT used in computing
    the numerical eigenvalues above.
    """
    p_range = range(-(n_eigs + 5), n_eigs + 6)
    predicted = sorted([k_v**2 + (p / eps) ** 2 for p in p_range])
...
def main() -> None:
    ...
    print(f"Hill spectrum: ε={eps}, χ={chi}, N={args.n_grid}, n_eigs={args.n_eigenvalues}")
    ...
    if args.compare:
        print()
        print("Comparison to zeroth-order analytical prediction μ² = k_v² + (p/ε)²:")

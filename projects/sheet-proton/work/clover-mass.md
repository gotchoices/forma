...
**Concern B — m_n/m_p and the joint fit with R_p pull in different directions.** Two operating points are in play:

- At ε ≈ 0.2 (zeroth-order m_n/m_p inversion): m_n/m_p fits to 0.03% (1.00168 vs observed 1.00138), but ω/Λ/Δ are missed in the PDG sweep, and R_major comes out too large.
- At ε ≈ 0.5 (PDG-fitness peak from `scripts/spectrum_vs_pdg.py --sweep`): R_major = 0.8444 fm matches R_p ≈ 0.84 fm within 0.5%; ω/Λ/Δ⁺⁺ are matched; but the m_n/m_p ratio under the (proton, neutron) = ((m_t, m_r) = (2, 1), (m_t, m_r) = (2, 2)) identification comes out poorly — the (2, 2) mode at 948.7 MeV is 9 MeV off from the observed neutron (and is closer to η' than to n).

Resolving this requires one of: (i) extending the (m_t, m_r) search range to find the right neutron mode at ε = 0.5; (ii) showing that χ-corrections substantively shift the ε ≈ 0.5 picture; or (iii) accepting that m_n/m_p and R_p genuinely require different identifications and the framework cannot fit both with one (ε, χ) and one identification scheme.

**Concern C — predicted R_major vs observed R_p — RESOLVED at ε = 0.5.** The PDG fitness-sweep peaks sharply at ε = 0.5 with R_major = 0.8444 fm, within 0.5% of the observed R_p = 0.84 fm. With no free parameters beyond the m_p calibration this is a third independent observable (alongside m_p and m_n/m_p) pointing to the same operating point — substantial support that the geometric picture is real signal rather than coincidence.

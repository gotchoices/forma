...
**Outcome: 2D-surface picture structurally rules out the hierarchy; 3D wave-guide extension recovers it qualitatively.** See [3-gen.md §12](3-gen.md) for the synthesis. Investigation proceeded in two stages: (1) a numerical attempt at finding compartmentalized band structure in the 2D Hill spectrum (sparse 15-point sweep, only ground-state-per-Bloch-sector readout; found no compartmentalized structure); (2) an analytical follow-on ([clover-modes-analytical.md](clover-modes-analytical.md)) that explained the negative numerical result structurally — in the 2D Hill equation, lobes are *wells* in the effective potential and lobe-localized states sit *below* the plane-wave continuum, opposite of the "smaller cavity → higher frequency" intuition. The natural extension to the 3D wave-guide picture ([tube-waveguide.md](tube-waveguide.md)) — treating the tube as a 3D solid torus with the clover as the cross-section — *does* produce the user's hoped-for hierarchy with mass ratios up to ~10²–10³ from cross-section asymmetry. Reading: gen-1 + gen-2 plausibly on the proton sheet via the 3D extension; gen-3 likely cross-sheet.

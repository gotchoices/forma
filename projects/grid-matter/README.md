...
- Both a **photon** (massless) and a **matter wave** (compact n=1, massive) interfere on the same lattice; the matter mode has a **longer de Broglie λ** (11.18 vs 8.07 nodes — solved from the dispersion *and* measured by FFT, 11.35 ± 0.48 vs 8.13 ± 0.25) and coarser fringes (period 32.4 ± 4.6 vs 25.9 ± 2.9 nodes), with a one-slit control giving a single lobe in both cases. *This is **classical** linear-wave interference (matter follows from photon by linearity) — staging, not yet distinctively quantum; λL/d is now consistent within the fringe measurement's ~1-bin resolution, where the earlier anisotropic run missed by a factor ~3 — a check passed, not a precision test, since the geometry is near-field (Fresnel number ≈ 2).* **[C]**

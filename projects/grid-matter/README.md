...
- Repo-wide survey: *no* prior forma study showed dynamical containment — which *motivates* (not deductively implies) the missing ingredient being a **focusing** nonlinearity (independently: focusing+saturating is the standard soliton recipe). (These negatives are load-bearing — they are the argument for Ch 2.)

**Ch 2 — The compact-phase *field value* is intrinsically focusing (the key result)**
- If the field *value* (not the coordinate — that's Ch 3) is a compact phase — a
...
**entanglement/Bell (Ch 9), which is open**. Ch 6 is classical wave; Ch 7's single
whole-quantum click is genuinely quantum but its |ψ|² distribution is assumed. So
Act 2 is a **question**, and Ch 9's toy is an arithmetic consistency check, not
physics progress.*
...
**Ch 8 — Measurement: an open ontology fork (not "no collapse, settled")**
...
Q-ball), Ch 5 (grid-duality ch.7). **Open [O]:** Ch 4 localized-3D particles;
Ch 9 entangled Born.

So the **matter half** rests on **three** honestly-named items: the Ch 2 phase-topology
**premise**, the cosine-from-scatter reduction **[P]**, and **localized-3D particle
stability [O]** (charged and neutral). Ch 3 (matter waves/mass/de Broglie) is
exact and premise-free. The **QM half** is derivation-ready through Ch 7 **modulo the

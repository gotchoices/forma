...
This file tracks items that remain open *beyond* the planned chapter refactor described in [work1.md](work1.md) (with corrections in [work1-review.md](work1-review.md)). The work1 refactor — once executed — will resolve TODO-M3 (two U(1)s puzzle), restructure Chapter 5 around the wrap-order-asymmetric standing-wave construction, and propagate the convention/labeling changes through the project's other chapters. The items below are *not* covered by that plan and need separate attention.
...
**Status:** Open. Simplified by the planned work1 refactor (which reduces the test from two U(1)s to one) but not resolved by it.
...
- Property 4: real calculation. Expand the geodesic equation for a charged particle on the surviving gauge potential h_μw (after work1's R_u-symmetrization), take the slow-motion limit, isolate the force term, show it has the structure of the Lorentz force. Smaller scope than the original two-U(1) version (only one gauge potential to test), but still a real calculation.

**Estimate:** Half-day, mostly Property 4.

**Suggested approach:** After the work1 refactor lands, write Property 4 as a new sub-subsection in the rewritten Ch 5 with the explicit calculation. This converts the chapter's gauge-potential claim from "asserted" to "derived for a single charged-test-particle on a closure-satisfying mode's metric perturbation."

---

### TODO-M8(a) — Chapter 8 optimization computation pending

**Status:** Open. Substantive work. Unaffected by the work1 refactor.
...
**Scope of fix:**
- Compute E(k; σ, ε, q) for k-component multi-link configurations under σ_uw shear (using the work1-refactor's labeling, this is k × T(m, 1) form).
...
**Suggested order:** Best done after the work1 refactor lands so the labeling is settled. Couples to TODO-L5 (which depends on knowing k_opt to identify "the quark configuration").

---

### TODO-L5 — Fractional-charge sign assignments not worked out

**Status:** Open. Acknowledged in Ch 8 §6.3 and in open-questions, but flagged as a substantive missing piece. Unaffected by the work1 refactor (which preserves fractional-charge predictions but doesn't address sign structure).
...
**Scope of fix:**
- Trace how the framework's (m, n) signs determine charge sign of each component (under work1's R_u-symmetrized construction, this is the sign of the tube-direction wavenumber n).
...
**Suggested order:** Couples to TODO-M8(a). Probably best done after both the work1 refactor and the Ch 8 optimization, so the configuration in question is settled.

---

### TODO-N2 — Infinite tower of mass-only states needs engagement

**Status:** Open. Identified as Light/Moderate by reviewer. Slightly reframed by work1 (genuine torus knots become "mass + chirality field" rather than pure mass-only), but the underlying tension with observation persists.

**What review.md says:** The closure rule predicts the entire genuine-torus-knot tower (T(2, 3), T(2, 5), T(3, 4), T(3, 5), T(2, 7), …) as closure-failing. Standard physics has only a small number of stable neutral massive species. The framework currently lists the prediction without engaging with the tension.
...
**Scope of fix (modest):** Add a short subsection or paragraph to the rewritten Ch 4 (post-work1) or Ch 9 acknowledging the tower and listing candidate selection mechanisms using the framework's existing machinery:
...
**Suggested order:** Easy. Best done after the work1 refactor lands so the closure-failing categorization is settled.
...
**My recommendation:** Decline to tighten. Possibly add one sentence at top of the rewritten Ch 4 (post-work1) cross-referencing Ch 3's terminology convention if it helps; even that is borderline.

---

## Cross-cutting observations

- The remaining substantive items (M2, M8(a), L5, N2) are all about *carrying through* what the framework already structurally predicts. None of them require new framework ingredients; they require honest computation against the existing structure (after the work1 refactor lands). This is the right kind of remaining work for a project that is mostly framing-complete.
- The matter/antimatter origin question — opened by Ch 6 §6's chirality finding and tightened under work1.md §11.4 (per work1-review.md item #4) — is the one place where the framework currently has *no* derived mechanism for a structural property the user expects to be physically meaningful. work1 sharpens what σ_uw *cannot* do (it cannot bias matter/antimatter, only chirality within particles). Where matter/antimatter bias does come from is forwarded to substrate-level (grid-primitive) work; the user has indicated they have ideas to discuss separately.

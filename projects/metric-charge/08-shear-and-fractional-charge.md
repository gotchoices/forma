...
Per [Ch 1 §10](01-foundation.md)'s classification, only T(m', 1) primitives and their k-component repetitions k × T(m', 1) are closure-satisfying. Bloch modes at (m, n) with the gcd-reduced primitive having tube winding |n'| > 1 are off the closure-satisfying lattice — read as harmonic content, composite states, or spatially separated multi-primitive configurations rather than as new fundamental sectors. The single-Bloch-mode commitment above applies to the closure-satisfying inventory; downstream development assumes the primitive (m, 1) form.

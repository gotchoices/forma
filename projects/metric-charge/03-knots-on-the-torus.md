...
- **Cycle swap:** T(p, q) ≡ T(q, p) topologically. Swapping cycles is a coordinate-relabel for *bare* topology. The (m, n) and (n, m) modes are nevertheless *different physical states* in metric-charge: the (u, w) ↔ (w, u) symmetry is broken by three asymmetric conventions introduced downstream — the closure condition ([Chapter 1 §10](01-foundation.md)) singles out w-winding specifically, the aspect-ratio convention ([Chapter 1 §3](01-foundation.md)) labels one direction "tube" and the other "ring," and the gauge convention ([Chapter 5](05-metric-self-consistency.md)) selects one of the two U(1) potentials as the physical photon.

These three conventions are *not independent*. They reduce to a single convention adopted in [Chapter 1 §10](01-foundation.md) — the **order in which we wrap the torus**, naming which compact direction is *tube* (where charges arise) and which is *ring* (where mass arises). Under (u, w) ↔ (w, u) swap with ε → 1/ε, the bare framework is symmetric; once the wrap-order convention is adopted, the three downstream conventions flip together under the swap. The explicit demonstration is in [Chapter 5 §6.5](05-metric-self-consistency.md). Whether the adopted convention matches the way the universe seems to work, or whether some structural mechanism forces it, is a downstream question for [grid-duality §8](../grid-duality/08-where-alpha-appears.md) and alpha-derivation work, not a determination this chapter makes.

(A note on σ_uw shear: earlier framings of this project listed σ_uw alongside the three wrap-order conventions as a fourth (u, w)-asymmetric convention. Chapter 5 §6.5 shows this is incorrect — σ_uw is symmetric in (u, w) and does not break the wrap-order symmetry; what it breaks is a sign-flip symmetry on the (m, n) labels. σ_uw is a structurally separate asymmetry-introducing mechanism, examined in [Chapter 6 §6](06-handedness-and-pairs.md) and [Chapter 8 §2](08-shear-and-fractional-charge.md).)
...
- **Spin candidates.** Observed pattern (independent of any specific external derivation): a 2-torus substrate appears to carry spin-1/2-like structure for single-knot configurations on it, with other spin values plausibly emerging from combinations of knots on multiple substrates (a multi-sheet question). The framework treats spin as a label that may itself be derived from the substrate's dimensionality and the knot's structure rather than as an independent quantum number — but the explicit geometric derivation is open work, deferred to future projects. The framework does not commit to any specific spin-derivation formula; it notes only that the observed correspondence between substrate dimensionality and spin tier is suggestive of a structural origin.
...
| "Weak knot" diagonal | (1, q) for q ≥ 1 (and the equivalent T(p, 1)) | **Unknot** (no genuine knotting in 3-space) | **Satisfies** — m = 1 trivially divides n | Closure-satisfying primitive; MaSt model-F proposes T(1, 2) as a candidate identification with what standard physics calls the electron |
...
- **The "weak knot" diagonal class** (T(1, q)) is topologically the unknot but satisfies the synchronization rule (m = 1 divides every n). These are the framework's *primitive* charged states. They include T(1, 2) — the (1, 2) winding ratio that MaSt model-F proposes as a candidate identification with what standard physics calls the electron. **A topologically trivial unknot can realize a charged-particle primitive as substantive as anything in the standard inventory.**
...
- **Does not derive spin from winding.** The framework's observed pattern is that spin tier appears to track substrate dimensionality (a 2-torus carrier corresponds to spin-1/2 in the cases compared so far); a geometric derivation of spin from torus structure is open work for future projects, not undertaken here. Spin is treated as an external label whose framework-side derivation is deferred.

...
- **Spin candidates.** [matter-from-light §4](../../papers/matter-from-light.md) argues that spin-1/2 of an electron arises geometrically from the 1:2 winding ratio of T(1, 2) via ℓ = E/ω = ℏ/2. This is an external derivation cited here; spin may itself be a label derived from (m, n) rather than an independent quantum number. (Note: T(1, 2) topologically is the unknot per §2, so the "1:2 ratio" is the sub-(m, n) phase relationship, not a topological knot — this nuance is part of why standing-wave phase alignment from §4 matters.)
...
| "Weak knot" diagonal | (1, q) or (p, 1) with both nonzero | **Unknot** (no genuine knotting in 3-space, but both windings nonzero) | **Eligible** — both windings nonzero | Closure-eligible despite topological triviality; matter-from-light identifies the electron with this class via T(1, 2) |
...
- **The "weak knot" diagonal class** (T(1, q) and T(p, 1)) is topologically the unknot but has both windings nonzero. The closure condition operates on windings, not on knot type, so these modes are closure-eligible. They include T(1, 2) — the (1, 2) winding ratio that [matter-from-light §4](../../papers/matter-from-light.md) identifies with the electron. **A topologically trivial unknot can be the geometric realization of an electron.** Worth highlighting because the topology-vs-physics distinction is most consequential here.
...
| How does one count "how many knots in a particle"? Is a proton a 3-component link or 3 independent knots or 1 knot with (m, n) ∝ (3, ·)? | metric-binding + downstream MaSt-correspondence work |

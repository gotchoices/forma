...
6. **Closure failure → mass without observable charge — three mechanisms.** A wave configuration that does *not* satisfy the closure rule carries mass but no observable EM:
   ...
   - **Chirality-non-degenerate** (genuine torus knots T(p, q) with p, q ≥ 2 and gcd = 1 — e.g., T(2, 3), T(2, 5), T(3, 4), ...) — fails by chirality non-degeneracy: the curve is chirally distinct from its mirror in 3-space
...
8. **Shear biases chirality within particles, not matter/antimatter populations.** σ_uw between u and w produces:
   ...
   - **Selection of multi-component link structure** — the energetics of k × T(m, 1) configurations under shear pick out a favored k_opt(σ, ε) (Chapter 8 §6)
...
4. **`04-the-closure-condition.md`** — When does a (massive) mode also carry observable EM charge? Work through the chirality criterion (with synchronization n | m as the operational test) explicitly. Identify which (m, n) modes satisfy it (charged-state modes) and which don't (mass-only modes — single-axis, chirality-non-degenerate, and cancellation-pair candidates for non-charged massive states).

...
where m4 (the largest of the quark-region dims) is the common hub.

Dim labels are size-ordered (smallest first):
  m1 ≈ 0.007 fm   — smallest ring; hosts the heaviest pair (t, b)
  m2 ≈ 0.91 fm    — middle ring; hosts (c, s)
  m3 ≈ 181 fm     — largest ring; hosts the lightest pair (u, d)
  m4 ≳ 5740 fm    — common tube/hub (plays tube in all 3 pairs)
...
Ma((1, 2), (1, 3), (2, 3)):
  ...
  wye:      shared dim (m4) is the LARGER (the L_tube role)
            → each pair has its own L_ring → 3 distinct mass scales

The fit: m4 plays "tube" (large circumference) in all 3 quark pairs;
m1, m2, m3 play "ring" (small circumference) each in one pair, giving the
three generation mass scales 1/L_1, 1/L_2, 1/L_3.
...
Free parameters: 4 L's (L_1, L_2, L_3, L_4) + 3 σ_eff (one per pair)
Constraints:     6 quark masses
Net: 7 unknowns for 6 equations → 1 DOF (the choice of L_4, which must be
...
def main() -> None:
    ...
    lines.append("=" * 90)
    lines.append("")
    lines.append("Topology: Ma((1, 4), (2, 4), (3, 4)) — m4 common to all 3 pairs.")
    lines.append("Assignment: m4 plays TUBE in all 3 pairs (large L);")
    lines.append("            m1, m2, m3 play RING (small L), one per pair.")
    lines.append("Size-ordering: m1 smallest → m4 largest of the quark-region dims.")
    lines.append("")
    ...
    L_1 = L_ring_per_pair[1]
    L_2 = L_ring_per_pair[2]
    L_3 = L_ring_per_pair[3]

    # Step 2: solve for L_4 minimum (pure-ring regime: L_T >> L_R/f for each pair)
    L_4_min = max(L_ring_per_pair[d] / f_per_pair[d]
                  for d in (1, 2, 3)) * 10  # 10× margin
    L_4 = max(L_4_min, 5000.0)

    lines.append("")
    lines.append(f"  L_4 (common tube) requires L_4 ≫ L_ring/f for each pair.")
    lines.append(f"  Strictest constraint: L_4 ≫ {L_ring_per_pair[3]/f_per_pair[3]:.4g} fm "
                 f" ⇒  use L_4 = {L_4:.4g} fm.")
    lines.append("")
    ...
    for pair_label, q_light, q_heavy, ring_dim in GEN_PAIRS:
        ...
        cases.append((pair_label, "T(1, 2)", q_light, L_4, L_R, 2 - sigma_eff, q_light))
        cases.append((pair_label, "T(1, 1)", q_heavy, L_4, L_R, 1 - sigma_eff, q_heavy))
    ...
    lines.append("  • Wye topology has all three pairs sharing L_T = L_4 (the larger),")
    lines.append("    so each pair has its own L_R (= L_1, L_2, L_3) giving its own mass scale.")
    ...
    lines.append("")
    lines.append(f"Quark-sector geometry (size-ordered, smallest first):")
    for d in (1, 2, 3, 4):
        L = L_ring_per_pair.get(d, L_4 if d == 4 else None)
        if d == 4:
            label = "L_4 (common tube)"
        else:

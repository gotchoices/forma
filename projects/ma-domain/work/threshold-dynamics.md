...
**Cross-references:**
- [grid-matter](../../grid-matter/) — the concrete computational testbed for this mechanism: a 1D-space + 1D-compact **(x,c) cylinder** in which head-on photons drive the substrate to **saturation**, redirecting energy S→c (pair production). Treats the saturation bound as *the* nonlinearity and proposes an energy-conserving **spillover** node (excess on a saturated edge spills into the orthogonal edge). This file's γ→e⁺e⁻ "production-from-injection" test is grid-matter's milestone M2.

...
- **All three electron σ_eff are fit-pinned near 2** across the 1-parameter manifold — the solver finds them invariant. But σ_eff is a *free per-sheet fit parameter*, not a structural constant: the report's "2.0000" is rounded; the true fitted values sit at 2 − (3–6)×10⁻⁴, differ between the e- and μ-sheets, and those ~10⁻⁴ deviations are load-bearing — they carry the e/μ/τ mass splits. The per-sheet shear freedom itself is load-bearing, not optional: the [uniform-shear test](../scripts/uniform_shear_test.py) re-fits K4 with the shear tied to a *single* value and cannot get the worst mass error below 97% (one bare shear σ) or 133% (one shared σ_eff, which the optimizer parks at exactly 2) — against machine precision when σ_eff is free per sheet ([outputs/uniform_shear_test.txt](../outputs/uniform_shear_test.txt)). σ_eff = 2 *exactly*, like any single shear value shared across the topology, is decisively excluded. The earlier reading — "a structural R53 output forced by the topology" — is withdrawn ([STATUS.md](STATUS.md) shear-cleanup).

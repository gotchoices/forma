...
A note on the strength of this commitment. Matched chirality is a *simplifying* assumption, not a derivation: nothing in the cylinder's microstructural picture forces the elastic and kinetic chiralities to coincide *exactly*. The chapters that follow take exact matching as given, because it produces the cleanest correspondence with vacuum Maxwell (chapter 6) and yields a tractable wave dynamics (chapter 2). [Chapter 9](09-chirality-asymmetry.md) revisits the assumption explicitly and develops what changes if a small mismatch is admitted — a structural option that may be relevant for downstream questions about anomalous magnetic moments and matter / antimatter asymmetry in [metric-charge](../metric-charge/).

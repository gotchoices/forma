...
A note on what "mass" means here, fleshed out in chapter 1 §8: the entries of *D* are not arbitrary coefficients we add to the wave equation. They have a natural origin under the fractal-microgrid interpretation of the cylinder primitive — waves circulating around the cylinder's cross-section have quantized transverse momentum (*p_cross* = ℏ*n*/*r*, the standard Kaluza-Klein quantization), and these transverse-momentum modes manifest as substrate inertia for waves propagating along the axis. The mechanism is **structurally identical** to the rest-mass story in [metric-mass](../../projects/metric-mass/), at a different scale: there, mass emerges from a compact dimension *u* at the Compton scale; here, it emerges from the cylinder circumference 2π*r* at the sub-Planck scale. The cylinder primitive's "mass" is therefore not a category error vs metric-mass — it is the same KK-style emergent quantity at a smaller scale (and "dark mass" in MaSt terminology, since the 1D circumference wrap can produce mass-analog inertia but not charge).

In GRID's natural units (*c* = ℏ = 1), the matched-chirality + bare-speed condition reduces *M* and *D* to the *same* matched-chirality matrix (chapter 1 §8). The separate-matrix presentation here is for pedagogical clarity — the matched-chirality condition is more visible when *D* and *M* are written out separately than when they are folded into one.

...
6. **Matched chirality + lattice cadence pin the cylinder's ratios.** The cylinder primitive's chirality manifests in both stiffness and inertia from a single microstructural source (chapter 1 §8 — *matched chirality*: *K_eφ*/√(*K_ee K_φφ*) = *D_eφ*/√(ρ *I_φ*) = χ̃). With matched chirality and bare-speed equality (*K_ee*/ρ = *K_φφ*/*I_φ* = *c*²), the matrices satisfy *M* = *c*² *D*, and the cylinder propagates *both* polarizations at *c* (no slow-mode split, no birefringence). The dimensionless stiffness ratios are pinned by the lattice cadence; an overall stiffness scale, cylinder length *L*, and cross-section radius *r* remain free. The slow-mode tension that appeared in earlier drafts (with diagonal *D*) is removed at the foundation level — it was a consequence of an inconsistent diagonal-*D* simplification, not a feature of the model. *[Chapter 3 — confirmed.]*

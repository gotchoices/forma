...
2. **The slow-mode interpretation.** Chapter 3 found a second propagating mode at *c*_− ≈ 0.414 *c* alongside the photon-speed fast mode at *c*. Vacuum Maxwell has only one speed; chapter 3 flagged three candidate resolutions (slow mode gaps out at lattice scale; lattice assembly averages the two speeds; slow mode is a real prediction of vacuum birefringence). Chapter 5 (lattice assembly) and chapter 6 (Maxwell bridge) are where this question is taken up.
...
5. **`05-the-2d-lattice.md`** — Assemble cylinder primitives into a 2D periodic lattice with node continuity. Derive the wave equation in 2D, find the collective modes, and **address the slow-mode tension flagged at the end of chapter 3**: does the slow mode (*c*_− ≈ 0.414 *c*) acquire a mass at lattice scale (becoming a non-propagating localized excitation), get averaged with the fast mode by lattice geometry, or persist as a real second wave that must be reconciled with vacuum Maxwell in chapter 6? Establish the framework that the Maxwell-bridge and α-derivation chapters use.

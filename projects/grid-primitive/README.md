...
8. **Chirality asymmetry χ_anti.** Chapter 9 develops a structural option in which matched chirality is admitted to be only approximate, parameterised by a small dimensionless χ_anti. The chapter establishes (mathematically) that this parameter is gauge-equivalent to zero on extended space (no conflict with macroscopic isotropy of *c*) and produces gauge-invariant Wilson-loop phases on compact wraps. The chapter offers (speculatively) anomalous magnetic moments and matter / antimatter sign asymmetry as candidate macroscopic interpretations of those phases, while emphasising that whether the speculative reading is correct is for downstream work to decide. What the chapter does *not* settle: whether χ_anti is non-zero in nature, whether the speculative interpretation actually accounts for any observed anomaly, what value χ_anti takes if so, and how it assigns to specific particle species through wrap-orientation conventions. These are downstream questions for [metric-charge](../metric-charge/) or its successors.
...
9. **`09-chirality-asymmetry.md`** — Revisit the matched-chirality assumption of chapter 1 §8 explicitly. The assumption was a *simplifying choice*, not a derivation; nothing in the cylinder's microstructural picture forces the elastic and kinetic chiralities to coincide exactly. This chapter parameterises a small antisymmetric perturbation by a single dimensionless χ_anti, derives its dispersion-level consequences (none, under the matched antisymmetric extension), identifies the perturbation at the matter-field level with a built-in substrate-level background gauge field *A_μ*, and traces the consequences through to the gauge sector (*E*, *B*). The interpretive content of the chapter is the standard Aharonov-Bohm distinction: the built-in *A_μ* is gauge-equivalent to zero on extended (open) space — invisible, no conflict with macroscopic isotropy of *c* — but produces gauge-invariant Wilson-loop phases on compact wraps. The chapter then offers anomalous magnetic moments and (*m*, *n*) ↔ (−*m*, −*n*) sign asymmetry as a *speculative* reading of those Wilson-loop phases — possible macroscopic observables, but interpretations to be tested in downstream work rather than results this project supports. The chapter does not pin χ_anti to a value, does not assign wrap orientations to specific particle species, and does not derive specific numerical predictions; it makes a structural option available that [metric-charge](../metric-charge/) or its successors may invoke if and when the matter-field-to-gauge-field calculation is carried through.

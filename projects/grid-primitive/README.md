...
**Status:** Chapters 1–4 complete. Chapters 5–9 in the roadmap. Three numerical fail-fast tests in [scripts/](scripts/) confirm the chapter-4 prediction (1/r force scaling on the 2D lattice).
...
2. **Shear is necessary for propagation.** With K_eφ = 0, strain and phase decouple and the cylinder supports no propagating waves. K_eφ > 0 is structurally required. *[Chapter 2 — confirmed.]*

3. **Stability bounds the shear.** The stiffness matrix must remain positive-definite, giving K_eφ² < K_ee · K_φφ. The upper limit χ̃ = 1 is degenerate (wave speed → 0); the natural propagating regime sits strictly inside (0, 1). *[Chapter 2 — confirmed.]*

4. **Equipartition picks the natural shear.** χ̃ = 1/√2 emerges as the geometric mean of the stable range (0, 1), at which the chiral coupling is well-engaged but the stability margin is substantial. *[Chapter 2 — confirmed (reframed from "equipartition" to the cleaner geometric-mean characterization).]*

5. **Bidirectional propagation symmetry holds despite shear.** Although shear makes the cylinder chiral and creates a handedness distinction in the *mode basis* (L-circular vs R-circular polarization), it does *not* create directional asymmetry in *propagation*: L-going and R-going waves travel at the same speed, and two pulses launched from opposite ends pass through each other and exit with their waveforms intact. *Negative result candidate:* if the algebra produces direction-dependent group velocity, the cylinder primitive is incompatible with vacuum Maxwell. *[Chapter 2 — confirmed; ω² depends on k² only.]*

6. **The lattice signal speed *c* + equipartition pins the cylinder's dimensionless ratios.** Imposing *c*_+ = *c* (the only viable identification — see chapter 3 §3) plus χ̃ = 1/√2 (theory 4) pins both *K_eφ*/*K* = 1/√2 and *K*/ρ = (2 − √2) *c*² in the simplifying special case ρ = *I_φ*, *K_ee* = *K_φφ*. The qualitative conclusion — two ratios pinned, three parameters free (overall stiffness scale, cylinder length *L*, cross-section radius *r*) — holds in the general case as well. The slow-mode speed *c*_− ≈ 0.414 *c* is determined alongside, raising a vacuum-Maxwell tension flagged for chapter 5–6 to resolve. *[Chapter 3 — confirmed.]*
...
2. **The slow-mode interpretation.** Chapter 3 found a second propagating mode at *c*_− ≈ 0.414 *c* alongside the photon-speed fast mode at *c*. Vacuum Maxwell has only one photon speed; the cylinder primitive does not, and the discrepancy needs resolution at the lattice or coarse-graining scale. Three non-mass-based candidate resolutions are in scope (the project does not interpret the slow mode as a "matter" particle — mass-from-compact-dimensions is the metric-mass story, not this project's): (i) **emergent Lorentz symmetry** — the two-speed split is a lattice-scale artifact that vanishes in the continuum limit; (ii) **longitudinal/non-radiative** — the slow mode coarse-grains to a gauge-fixable analog of Maxwell's longitudinal/Coulomb component, which does not propagate in the gauge-fixed theory; (iii) **lattice-scale gap** — the slow mode acquires a gap of Planck-scale energy at lattice scale, decoupling completely at observable wavelengths (the 18-orders-of-magnitude scale separation between Planck length and any observable scale buries lattice-scale gaps). Chapter 5 (lattice assembly) examines (iii) directly; chapter 6 (Maxwell bridge) handles (i) and (ii).
...
5. **`05-the-2d-lattice.md`** — Assemble cylinder primitives into a 2D *hexagonal* lattice (per [grid/hexagonal.md](../../grid/hexagonal.md), giving ζ_2D = 1/3) with wye node connectivity. Derive the wave equation in 2D, examine the band structure, and **address the slow-mode tension flagged at the end of chapter 3**: does the slow mode (*c*_− ≈ 0.414 *c*) acquire a Planck-scale gap (decoupling completely at observable wavelengths via the 18-orders-of-magnitude scale separation), coarse-grain to a non-propagating longitudinal/non-radiative degree of freedom (resolved in chapter 6), or persist as an open anomaly? Establish the framework that the Maxwell-bridge and α-derivation chapters use.

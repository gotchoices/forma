...
**Status:** Chapters 1–10 complete (project closed). Chapter-4 verdict: Scattering is the substrate's dynamics; gravity emerges from the substrate's graph Laplacian directly ([04-model-comparison.md](04-model-comparison.md)). Chapter-5 verdict: real-valued Scattering is the effective theory of a binary-bit substrate at Planck scale, recovered via spatial averaging under stochastic rounding (M ≥ (amp_max/((N−1)·ε))² cells per macroscopic window; the minimum per-cell resolution is 1 bit). Chapter 6: Scattering generalizes to any coordination and any dimension by the algebraic properties of S = (2/N)·J − I. Chapter 7: each lattice closure (open chain → ring → plaquette → torus) carries a physical phenomenon (substrate → light → mass → charge) by accreting one new conserved observable per wrap; periodicity is structurally preferred at every rung including L0 → L1; a speculative L4 third winding is identified as a structural extension if the universe is a 3-torus. Chapter 8: α is structurally located at L3 (the first rung where the substrate has been wrapped twice, providing the U(1) × U(1) cross-coupling structure α requires); α is plausibly a categorical invariant of the wrap-of-a-wrap operation rather than a geometric ratio of lattice constants; specific value 1/137 is open work needing topological / categorical / RG machinery beyond this project's scope. Chapter 9: a node's functional behavior can be constructed from bare edges plus *cooperative context* (the triangle construction: three inner micro-edges performing Givens rotations whose angles are jointly chosen to compose to S, with no central register or coordinator at runtime — cooperation built into the angle parameters at construction time). Edges are more fundamental than nodes, but the substrate's minimal structure is *bare edges + connectivity + cooperative context*; cooperation is a real and fundamental third ingredient. Chapter 10: closing summary — established results consolidated, ruled-out items documented, unexpected findings noted, comparison with grid-primitive's analog-first cylinder primitive shows strong convergence on substrate structure despite opposite starting points.
...
├── 09-node-decomposition.md        functional model of node from edges + context
├── 10-closing-summary.md           closing summary
